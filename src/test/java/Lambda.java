...
//        могут использовать переменные определенные в текущем классе, но переопределять их не могут.
        ...
        numbers.takeWhile(n -> n < 0)
                .forEach(n -> System.out.println(n));  //результат -3, -2, -1

        //Метод dropWhile() выполняет обратную задачу - он удаляет ВСЕ элементы потока которые соответствуют условию.
        Stream<Integer> numbers2 = Stream.of(-3, -2, -1, 0, 1, 2, 3, -4, -5, 6, 7);
        numbers2.sorted().dropWhile(n -> n < 0)
                .forEach(System.out::println); //результат 0, 1, 2, 3, 6, 7
